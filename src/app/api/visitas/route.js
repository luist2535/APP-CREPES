...
export async function POST(request) {
  try {
    ...
    const { 
      ...
      evento_id, repuestos, firma_auxiliar, hallazgos, acciones_correctivas
    } = body;
    ...
    const result = db.prepare(`
      INSERT INTO visitas (pdv_id, user_id, area_id, tipo_visita_id, plantilla_id, fecha, hora_inicio, hora_fin, datos_formulario, responsable_id, fecha_compromiso, estado, observaciones, repuestos, firma_auxiliar, hallazgos, acciones_correctivas, evento_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'completada', ?, ?, ?, ?, ?, ?)
    `).run(
